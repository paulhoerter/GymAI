...
def draw_3d_skeleton(ax, keypoints_3d, connections):
    ...
    for start, end in connections:
        if start < len(keypoints_3d) and end < len(keypoints_3d):
            ...
            # Plot (X, Z, Y) -> Z est mappé sur l'axe de profondeur visuel
    ...
    # REGLAGES CRITIQUES POUR L'ASPECT RATIO
    # Hauteur (Z_plot) : on garde grand pour voir le corps entier
    ax.set_zlim([-0.9, 0.9]) 
    
    # Profondeur (Y_plot) : on réduit les limites pour "zoomer" horizontalement (étirer le profil)
    # Plus cette valeur est petite, plus le profil sera large à l'écran
    depth_zoom = 0.4 
    ax.set_ylim([-depth_zoom, depth_zoom])
    
    # Largeur (X_plot) : peu importe en vue de profil pur
    ax.set_xlim([-0.8, 0.8])
    
    # Force un ratio qui remplit la boite
...
def process_video(video_path):
    ...
    t_width = 384
    t_height = int(height * t_width / width)
    
    output_path = video_path.replace('.mp4', '_squat_wide.mp4')
    out = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (t_width * 3, t_height))
    ...
    while cap.isOpened():
        ...
        if flexion_angle is not None:
            is_valid = flexion_angle < 90
            cv2.putText(canvas_2d, f"{flexion_angle:.0f}deg", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0) if is_valid else (0,0,255), 2)
        else:
            cv2.putText(canvas_2d, "--deg", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (100,100,100), 2)
        
        combined = np.hstack([frame_s, canvas_2d, canvas_3d])
        out.write(combined)
...
if __name__ == "__main__":
    process_video(r"C:\Users\paulh\Downloads\rachelle.mp4")